#################################################################################################################
# Gráficos com dados de mobilidade do Google
  ...
  if (opcao == "diario")
    for (i in 1:length(lista_regioes)){
      ...
      ggplot(dados_graf, aes(x = Data, y = value, color = name)) + 
        ...
        ylab("") +
        xlab("Data") +
        labs(title = lista_regioes[i], subtitle = "Dados diários - var% sobre o valor de referência",
             caption = "Fonte: Google   Valor de referência: valor mediano do período de cinco semanas entre 3 de janeiro a 6 de fevereiro de 2020") +
  ...
  if (opcao == "mensal")
    for (i in 1:length(lista_regioes)){
      ...
      ggplot(dados_graf_mensal, aes(x = Data, y = value, color = name)) + 
        ...
        ylab("") +
        xlab("Data") +
        labs(title = lista_regioes[i], subtitle = "Dados mensais - var% sobre o valor de referência",
             caption = "Fonte: Google   Valor de referência: valor mediano do período de cinco semanas entre 3 de janeiro a 6 de fevereiro de 2020") +
        scale_x_date(breaks = date_breaks("1 month"), labels = date_format("%d/%b")) + 
        theme(legend.position="bottom") +
        scale_color_discrete(name = "Colunas") + 
    ...
    if (opcao == "mm")
      for (i in 1:length(lista_regioes)){
        ...
        ggplot(dados_mm, aes(x = Data, y = value, color = name)) + 
          ...
          ylab("") +
          xlab("Data") +
          labs(title = lista_regioes[i], subtitle = "Média móvel de 7 dias, var% sobre o valor de referência",
               caption = "Fonte: Google   Valor de referência: valor mediano do período de cinco semanas entre 3 de janeiro a 6 de fevereiro de 2020") +
          scale_x_date(breaks = date_breaks("1 month"), labels = date_format("%d/%b")) + 
          theme(legend.position="bottom") +
          scale_color_discrete(name = "Colunas") + 
